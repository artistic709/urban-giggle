...
contract HakkaIntelligence {
    ...
    event Submit(address indexed player, uint256 stake);
    event Reveal(address indexed player, uint256[] submission, uint256 score);
    ...
    function submit(uint256 stake, bytes32 commitHash) public {
        Player storage player = players[msg.sender];

        require(now <= periodStart, "too late");
        require(commitHash != bytes32(""), "invalid input");
        require(player.commitHash == bytes32(""), "already submitted");

        totalStake = totalStake.add(stake);
        player.stake = stake;
        player.commitHash = commitHash;
        token.safeTransferFrom(msg.sender, address(this), stake);
        emit Submit(msg.sender, stake);
    }

    function reveal(address _player, uint256[] memory submission, uint256 salt) public returns (uint256 score) {
        ...
        bytes32 hash = keccak256(abi.encodePacked(submission, salt));
        require(hash == player.commitHash, "submission mismatch");

        score = innerProduct(answer, submission);
        ...
        emit Reveal(_player, submission, score);
    }
